...
void setup() {
  ...
  frameRate(60);
  smooth();
  file = new SoundFile(this, "brainBlast.wav");
  ...
  channels[10] = new Channel("High Gamma", color(23, 26, 153), "Cognition/Memory");
...
void serialEvent(Serial p) {
  ...
  if (incomingValues.length > 1) {
    ...
    if (packetCount > 3) {
     ...
     soundCheck = Integer.valueOf(incomingValues[1]);
     signalCheck = Integer.valueOf(incomingValues[0]);
       
      //make sound file play
      
      if((soundCheck >50) && (signalCheck ==0)){
        if(file.isPlaying()){
          startS = 1;
        }
        if(startS == 0){
